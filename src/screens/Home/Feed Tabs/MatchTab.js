...
const MatchTab = () => {
  ...
  const [SEARCHMATCHES, {loadingSearch, errorSearch, dataSearch}] =
    useLazyQuery(SEARCH_MATCHES);
  ...
  const [createRequestMutation] = useMutation(CREATE_MATCH_REQUEST);
  ...
  const handleRequest = async matchId => {
    try {
      ...
      Toast.show({
        type: 'success',
        text1: 'Match Request',
        text2: 'The match Request has been successfully sent.',
      });*/
      setRequestedItems(prevItems => [...prevItems, matchId]);
    } catch (error) {
      console.error(error);
    }
  };
      ...
      {showFilter && (
        <View style={styles.filterContainer}>
          <View style={styles.positionFilter}>
            <Text style={styles.positionFilterLabel}>
              Min Duration: ({minutesToTime(minDuration)})
            </Text>
            <Slider
              style={styles.slider}
              minimumValue={30}
          ...
          <View style={styles.positionFilter}>
            <Text style={styles.positionFilterLabel}>
              Max Duration: ({minutesToTime(maxDuration)})
            </Text>
            <Slider
              style={styles.slider}
              minimumValue={30}
          ...
          <View style={styles.positionFilter}>
            ...
            <TouchableOpacity
              ...
              <Text style={{color: '#999'}}>{dateFromLabel}</Text>
              <Ionicons
            ...
            <TouchableOpacity
              ...
              <Text style={{color: '#999'}}>{dateToLabel}</Text>
              <Ionicons
                name="calendar-outline"
                size={20}
                color="#999"
